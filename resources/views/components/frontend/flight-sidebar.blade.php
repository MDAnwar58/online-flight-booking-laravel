...
<div class="sidebar col-md-3">
    <div class="card card-body">
        <form class="row" action="{{ route('flight') }}" method="GET">
            <div class="col-md-12">
                {{-- <label for="priceRange" class="form-label text-capitalize text-primary fs-5 fw-semibold">one
                    way</label>
                <input type="range" name="price" onchange="flightPriceRange(event)" class="form-range" min=""
                    max="8000" step="1" id="priceRange">
                <div class="text-muted text-end pb-2">
                    <span><small id="price">0</small><small>tk</small></span>
                </div> --}}

                <div class="content">
                    <input type="range" name="price" min="2000" step="1" max="12000" value="" data-rangeslider>
                </div>
                <div class="text-center py-2">
                    <span class="price-up-down"></span> tk
                </div>
            </div>
            <hr>
            <div class="col-md-12">
                <h6 class="fs-5 fw-semibold text-primary pb-1">Airlines</h6>
                {{-- onchange="airline(event, '3000', 'SudiAirlines')" --}}
                @foreach ($airlines as $airline)
                    <div class="form-check mb-2">
                        <input class="form-check-input" name="airlines[]" id="SudiAirlines" type="checkbox"
                            value="{{ $airline->id }}" id="flexCheckDefault">
                ...
                @endforeach
                {{-- <div class="form-check mb-2">
                    <input class="form-check-input" name="airlines[]" id="UAEAirlines" type="checkbox"
                        value="UAE Airlines" id="flexCheckDefault">
                    <label class="form-check-label text-muted"
                        for="flexCheckDefault">
                        <span>UAE Airlines</span>
                    </label>
                </div>
                <div class="form-check mb-2">
                    <input class="form-check-input" name="airlines[]" id="AfganiAirlines" type="checkbox"
                        value="Afgani Airlines" id="flexCheckDefault">
                    <label class="form-check-label text-muted"
                        for="flexCheckDefault">
                        <span>Afgani Airlines</span>
                    </label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" name="airlines[]" id="PalastainesAirlines" type="checkbox"
                        value="Palastaines Airlines" id="flexCheckDefault">
                    <label class="form-check-label text-muted"
                        for="flexCheckDefault">
                        <span>Palastaines Airlines</span>
                    </label>
                </div> --}}
            </div>
            <div class="col-md-12 py-3 text-center">
                <button type="submit" class="btn btn-primary">Fliter</button>
                <a href="{{ route('flight') }}" class="btn btn-outline-danger">Clear Filter</a>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function(a) {
        ...
        function g(a, b) {
            ...
            var d = f(a),
                ...
                h = a[b];
            if (e) {
                for (var i = 0; e > i; i++) g[i] = d[i].style.cssText, d[i].style.setProperty ? d[i].style
                    .setProperty("display", "block", "important") : d[i].style.cssText +=
                    ";display: block !important", d[i].style.height = "0", d[i].style.overflow = "hidden", d[i]
                    .style.visibility = "hidden", c(d[i]);
                h = a[b];
        ...
        return j.prototype.init = function() {
            ...
            a = a || !1, a && (this.min = h(this.$element[0].getAttribute("min"), 0), this.max = h(this
                    .$element[0].getAttribute("max"), 100), this.value = h(this.$element[0].value, Math
                    .round(this.min + (this.max - this.min) / 2)), this.step = h(this.$element[0]
                    .getAttribute("step"), 1)), this.handleDimension = g(this.$handle[0], "offset" + i(this
                    .DIMENSION)), this.rangeDimension = g(this.$range[0], "offset" + i(this.DIMENSION)), this
                .maxHandlePos = this.rangeDimension - this.handleDimension, this.grabPos = this
                .handleDimension / 2, this.position = this.getPositionFromValue(this.value), this.$element[0]
                .disabled ? this.$range.addClass(this.options.disabledClass) : this.$range.removeClass(this
                    .options.disabledClass), this.setPosition(this.position, b)
        }, j.prototype.handleDown = function(a) {
            if (this.$document.on(this.moveEvent, this.handleMove), this.$document.on(this.endEvent, this
                    .handleEnd), !((" " + a.target.className + " ").replace(/[\n\t]/g, " ").indexOf(this.options
                    .handleClass) > -1)) {
    ...
    $(function() {
        $('input[type="range"]').rangeslider({
            polyfill: false,
            onInit: function() {
                $('.price-up-down').text($('input[type="range"]').val());
            },
            onSlide: function(position, value) {
                //console.log('onSlide');
                //console.log('position: ' + position, 'value: ' + value);
                $('.price-up-down').text(value);
            },
